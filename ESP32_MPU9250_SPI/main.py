...
class BLE():
    ...
    def ble_irq(self, event, data):
        if event == 1:
            '''Central disconnected, device connected to central'''
            self.connected()
    ...
    def run(self):
        ...
        while True:
            if(self.isConnect):
                ...
                # Gestion JSON para enviar data por BLE.
                self.jsonFiles += 1 
                self.jsonLocal = {"id": self.jsonFiles,
                                  "x": x,
                                  "y": y,
                                  "z": z}
                self.jsonSend = ujson.dumps(self.jsonLocal)
                
                # Enviando JSON-STRING
                self.sendData(self.jsonSend)
                print(self.jsonSend)
                #sleep_ms(5)
            else:
                self.jsonFiles = 0
